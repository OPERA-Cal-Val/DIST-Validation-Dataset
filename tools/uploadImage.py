...
def uploadImage(filePaths, metaData, awsSession):
    ...
    for key in filePaths:
        ...
        s3_key_pending = 'pending/files/'+now+'_imagecalc/'+os.path.basename(filePaths[key])
        s3_keys.append(s3_key_pending)
    bucket_name_staging = 'opera-calval-database-dist-staging'
    ...
    s3object = s3.Object(bucket_name_staging, 'pending/'+now+'_image.geojson')
    ...
    return 'pending/'+now+'_image.geojson'
